...
**Rendering** (`include/io/output/`, `src/io/output/`): `RenderStack` is an abstract base wrapping one ncurses `WINDOW*`, with `doRender(const RenderState&)` as its pure-virtual draw hook — layers hold no live references into `Game`'s object graph, only geometry from their constructor. `Renderer` holds layers in a z-ordered map and calls `compose(const RenderState&)` each frame, which runs each layer's `doUpdate()` (per-frame state hook, default no-op — currently unused by any layer, but kept as general infrastructure) then `doRender(state)`. Layers, added in `Game`'s constructor: `MapLayer` → `EntityLayer` (draws only what's in the player's FoV) → `HUDLayer` → `DebugLayer` (compiled in only when `NDEBUG` is not defined, i.e. debug builds). `Game::render()` builds a fresh `RenderState` snapshot every frame via `render_state_builder::build()` (`core/render_state_builder.h`/`.cpp`) and pushes it through `renderer_.compose()`. Render-effect state such as the player hit-flash now lives on `Game` (`playerHitFlashFramesRemaining_`), not on any layer — the builder resolves it into the snapshot's `RenderState::player` tint fields each frame, so no raw pointer into a layer exists anymore. `core/colors.h` keeps the plain `ColorPair` enum as shared vocabulary; `io/output/colors.h` holds the ncurses-specific `colorAttr()`/`initColors()`.
