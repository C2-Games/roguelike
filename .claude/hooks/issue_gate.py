...
Every change in this repo traces to an issue (.github/ISSUE_TEMPLATE has
blank_issues_enabled: false, and PULL_REQUEST_TEMPLATE.md carries a `Closes #`
line). This enforces that mechanically instead of relying on remembering it.

/start-issue records the active issue(s) in .claude/.current-issue; until it
does, Edit and Write on tracked files are denied.
...
def main():
    ...
    if not record:
        return deny(
            ...
            "first (open one from a template at "
            "https://github.com/C2-Games/roguelike/issues/new/choose if "
            "none covers this work).".format(rel)
        )

    branch = git_branch()
    if branch == MAIN_BRANCH:
        return deny(
            "Refusing to edit `{}` on `main`. Work happens on a "
            "`<type>/<description>` branch -- run `/start-issue <number>` to "
            "create one.".format(rel)
        )
