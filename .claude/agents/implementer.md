...
- Do not run `scripts/ci-local.sh`, formatting, or the build — you have no `Bash` tool, and
  `/check` is a separate step the dispatching session runs once across all changes.
...
this repo, but do not attempt it regardless of how the task is phrased.
